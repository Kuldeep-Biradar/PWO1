...
def generate_schedule(jobs_data, cleaning):
    ...
    for job_id, job in enumerate(jobs_data):
        ...
        for task_id, task in enumerate(job):
            ...
            machine_id = task[0][1]
            min_start_ends[(min_id, machine_id, job_id, task_id)] = [start, end]
    ...
    for (min_id, machine_id, job_id, task_id), (start, end) in min_start_ends.items():
        l_changeovers = []
        durations = []
        if machine_id != 0:
            continue
        alt_suffix = f"_j{job_id}_t{task_id}_m{machine_id}_min{min_id}_clean"
        l_last = model.NewBoolVar("laststep")
        ...
        #         model.Add(l_clean_duration == 3).OnlyEnforceIf(l_changeover)
        l_interval = model.NewIntervalVar(
            l_clean_start, l_clean_duration, l_clean_end, "interval" + alt_suffix
        )
        ...
        for (
            ...
            start_job_id,
            start_task_id,
        ), [start_2, end_2] in min_start_ends.items():
            if (
                start_machine_id != machine_id
                or start_job_id == job_id
            ):
                continue
            model.Add(l_clean_start >= end_2).OnlyEnforceIf(l_last)
            l_changeover = model.NewBoolVar("changeover" + alt_suffix)
            model.Add(start_2 == l_clean_end).OnlyEnforceIf(l_changeover)
            if start_min_id != min_id:
                model.Add(l_clean_duration >= cleaning.get((machine_id, min_id), 0)).OnlyEnforceIf(l_changeover)
            l_changeovers.append(l_changeover)
        intervals_per_resources[machine_id].append(l_interval)
        model.Add(sum(l_changeovers + [l_last]) == 1)
...
jobs = [
    [  # M07A5
        ...
        [(10, 1, "UF7", "M07A5"), (8, 2, "UF5", "M07A5")],  # task 1
    ],
    [  # B05Y5
        [(10, 0, "R368", "B05Y5")],  # task 0
        [(7, 1, "UF7", "B05Y5"), (8, 2, "UF5", "B05Y5")],  # task 1
    ],
]
...
out = generate_schedule(jobs_data, cleaning)
